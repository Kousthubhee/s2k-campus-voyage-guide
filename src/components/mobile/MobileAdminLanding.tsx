...
interface MobileAdminLandingProps {
  onAccessGranted: () => void;
}

export function MobileAdminLanding({ onAccessGranted }: MobileAdminLandingProps) {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-purple-900 to-slate-900 flex items-center justify-center p-4 safe-area-inset">
      <div className={`w-full ${isMobile ? 'max-w-sm' : 'max-w-md'}`}>
        <div className="text-center space-y-6">
          {/* Lock Icon with Mobile-Optimized Size */}
          <div className="flex justify-center">
            <div className="p-4 rounded-full bg-gradient-to-br from-purple-500/20 to-blue-500/20 backdrop-blur-sm border border-purple-400/30">
              <Lock className={`${isMobile ? 'h-10 w-10' : 'h-12 w-12'} text-purple-300 animate-pulse`} />
            </div>
          </div>

          {/* Mobile-Optimized Title */}
          <div className="space-y-4">
            <h1 className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-bold text-white leading-tight`}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="relative">
              <Input
                ...
                className={`
                  w-full px-6 py-4 ${isMobile ? 'text-base' : 'text-lg'} text-center
            ...
            {error && (
              <div className={`text-red-400 text-sm font-medium text-center p-3 rounded-lg bg-red-500/10 border border-red-500/20 ${isShaking ? 'animate-bounce' : ''}`}>
                {error}
              </div>
            )}
